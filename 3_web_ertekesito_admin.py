...
if funkcio == "📱 Raktári Kiszedés":
    ...
    if st.button("📥 Összes exportálása"):
        ...
        with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
            start_row = 0
            for w in ["M", "W", "XW", "XXW"]:
                get_matrix(adatok, w).to_excel(writer, sheet_name="Keszlet", startrow=start_row, index=False)
                start_row += 15
    ...
    if st.sidebar.text_input("Jelszó:", type="password") == ADMIN_JELSZO:
        ...
        naplo = [d.to_dict() for d in db.collection("naplo").where("datum", "==", datetime.now().strftime("%Y-%m-%d")).stream()]
        if naplo: st.table(pd.DataFrame(naplo))
