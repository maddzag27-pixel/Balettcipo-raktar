...
if funkcio == "📱 Raktári Kiszedés":
    ...
    # 1. Speciális készlet (korábbi kódod...)
    
    # --- EZT A FÜGGVÉNYT TEDD IDE A CIKLUS ELÉ ---
    def szinezo(row):
        k = row["Keménység"]
        if k == "ÖSSZESEN": 
            return ['background-color: #f0f0f0; font-weight: bold; text-align: center'] * len(row)
        return [f'background-color: {kemenyseg_szinek.get(k, "#FFFFFF")}; text-align: center'] * len(row)
    # ---------------------------------------------
